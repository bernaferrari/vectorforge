...
export async function fetchMaterialSymbolIcon(
  ...
  const syntheticOffSlash = Boolean(options.syntheticOffSlash && symbolName.endsWith('_off'));
  ...
  if (syntheticOffSlash) {
    ...
    if (!baseResponse.ok) throw new Error(`Material Symbol "${baseName}" was not found.`);
    const svgContent = appendVectorForgeSlash((await baseResponse.text()).trim());
