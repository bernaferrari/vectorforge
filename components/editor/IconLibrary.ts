...
const COMMON_MATERIAL_SYMBOL_NAMES = [
  "home",
  "search",
  "settings",
  "person",
...
export function getMaterialSymbolNames(): string[] {
  ...
  const names = new Set(COMMON_MATERIAL_SYMBOL_NAMES)
  for (const pair of MATERIAL_WIPE_READY_PAIRS) {
    names.add(pair.enabled)
    names.add(pair.disabled)
  }
