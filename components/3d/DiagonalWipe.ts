...
const wipeTravelRange = (width: number, height: number, axis: THREE.Vector2) => {
...
const clipRectangleWithHalfPlane = (
  ...
  if (output.length > 1 && output[0].distanceToSquared(output[output.length - 1]) < 0.0001) {
    output.pop();
  }
...
const boundaryLineForThreshold = (
  ...
  const addIfInBounds = (point: THREE.Vector2) => {
    const inBounds =
      ...
      point.y <= halfHeight + EPSILON;
    if (!inBounds) return;
    if (points.some((candidate) => candidate.distanceToSquared(point) < 0.0001)) return;
    points.push(point);
  };

  if (Math.abs(axis.y) > EPSILON) {
    addIfInBounds(new THREE.Vector2(-halfWidth, (threshold - axis.x * -halfWidth) / axis.y));
    addIfInBounds(new THREE.Vector2(halfWidth, (threshold - axis.x * halfWidth) / axis.y));
  }
  if (Math.abs(axis.x) > EPSILON) {
    addIfInBounds(new THREE.Vector2((threshold - axis.y * -halfHeight) / axis.x, -halfHeight));
    addIfInBounds(new THREE.Vector2((threshold - axis.y * halfHeight) / axis.x, halfHeight));
  }
...
export const createDiagonalWipePlanes = ({
  ...
  const boundary = boundaryLineForThreshold(safeWidth, safeHeight, axis, threshold);
...
export const buildDiagonalWipeClipPath = ({
  ...
  const adjustedProgress = clamp01((clamp01(progress) * travelDistance + Math.max(0, seamOverlap)) / travelDistance);

  if (adjustedProgress <= 0) return 'polygon(0% 0%, 0% 0%, 0% 0%, 0% 0%)';
  if (adjustedProgress >= 1) return 'polygon(0% 0%, 100% 0%, 100% 100%, 0% 100%)';

  const threshold = wipeBoundaryThreshold(safeWidth, safeHeight, adjustedProgress, axis);
  const points = clipRectangleWithHalfPlane(safeWidth, safeHeight, axis, threshold);
  if (points.length === 0) return 'polygon(0% 0%, 0% 0%, 0% 0%, 0% 0%)';

  return `polygon(${points
    .map((point) => `${(point.x / safeWidth) * 100}% ${(point.y / safeHeight) * 100}%`)
    .join(', ')})`;
};
