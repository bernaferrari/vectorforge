...
export function ColorPicker({
  ...
  const rootOutsidePointerRef = React.useRef<{ x: number; y: number } | null>(
    null
  )
  const stopOutsidePointerRef = React.useRef<{ x: number; y: number } | null>(
    null
  )
  React.useEffect(() => {
  ...
  React.useEffect(() => {
    if (activeStop >= normalizedStops.length)
      setActiveStop(Math.max(0, normalizedStops.length - 1))
    if (openStopEditor !== null && openStopEditor >= normalizedStops.length)
      setOpenStopEditor(null)
  }, [activeStop, normalizedStops.length, openStopEditor])

  const updateStops = React.useCallback(
    (nextStops: Array<{ id?: string; color: string; position: number }>) => {
      const usedIds = new Set<string>()
  ...
  const shuffleMeshPoints = React.useCallback(() => {
    ...
    const positions = normalizedStops
  ...
  const hexInputValue = React.useMemo(() => {
    ...
    const alphaHex = Math.round(Math.max(0, Math.min(1, activeAlpha)) * 255)
      .toString(16)
      .padStart(2, "0")
    return activeAlpha < 1
      ? `#${normalizedHex}${alphaHex}`
      : `#${normalizedHex}`
  }, [hex, activeAlpha])
  React.useEffect(() => {
  ...
  const colorAtPosition = React.useCallback(
    (position: number) => {
  ...
  const addStopAtRailPosition = (clientX: number, clientY?: number) => {
    ...
    const sampledColor = colorAtGradientPoint(
      gradientType,
      normalizedStops,
      point,
      primaryHex
    ...
    const nextStops = [
      ...normalizedStops,
      { color: sampledColor, position },
    ].sort((a, b) => a.position - b.position)
    const nextIndex = nextStops.findIndex(
      (stop) =>
        Math.abs(stop.position - position) < 0.0005 &&
        stop.color.toLowerCase() === sampledColor.toLowerCase()
    )
    updateStops(nextStops)
    setActiveStop(Math.max(0, nextIndex))
  ...
  const handleTextChange = (val: string) => {
    ...
    if (hexMatch) {
      ...
      if (ENABLE_ALPHA && hexMatch[2]) {
        activeOnAlphaChange(
          Number((parseInt(hexMatch[2], 16) / 255).toFixed(3))
        )
      }
    }
  }
              ...
              >
                ...
                {visibleStops.map((stop) => (
                  <button
                    ...
                    onPointerDown={(event) =>
                      handleStopPointerDown(stop, event)
                    }
                    className={cn(
                  ...
                  >
                    <span
                      ...
                      style={{ backgroundColor: normalizedStops[stop].color }}
                    />
                  </button>
                ))}
              </div>
                ...
                <div className="grid h-6 grid-cols-[1fr_28px] items-center gap-1 px-2">
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      const nextStops = [
                        ...normalizedStops,
                        { color: colorAtPosition(position), position },
                      ].sort((a, b) => a.position - b.position)
                      updateStops(nextStops)
                      const nextIndex = nextStops.findIndex(
                        (stop) => Math.abs(stop.position - position) < 0.0005
                      )
                      setActiveStop(Math.max(0, nextIndex))
                      setOpenStopEditor(Math.max(0, nextIndex))
                    }}
